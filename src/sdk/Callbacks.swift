...
public extension DescopeEnchantedLink {
    ...
    ///
    /// The caller should use the returned ``EnchantedLinkResponse`` object to show the
    /// user which link they need to press in the enchanted link text message, and then
    /// use the `pendingRef` value to poll until the authentication is verified.
    ///
    /// - Important: Make sure a default Enchanted Link URL is configured
    ///     in the Descope console, or provided by this call.
    ///
    /// - Parameters:
    ///   - phone: The phone number to send the enchanted link to.
    ///   - details: Optional details about the user signing up.
    ///   - redirectURL: Optional URL that will be used to generate the magic link.
    ///     If not given, the project default will be used.
    ///
    /// - Returns: An ``EnchantedLinkResponse`` object with the `linkId` to show the
    ///     user and `pendingRef` for polling for the session.
    func signUpWithPhone(_ phone: String, details: SignUpDetails?, redirectURL: String?, completion: @escaping @Sendable (Result<EnchantedLinkResponse, DescopeError>) -> Void) {
        Task {
            do throws(DescopeError) {
    ...
    /// - Returns: An ``EnchantedLinkResponse`` object with the `linkId` to show the
    ///     user and `pendingRef` for polling for the session.
    func signInWithPhone(_ phone: String, redirectURL: String?, options: [SignInOptions], completion: @escaping @Sendable (Result<EnchantedLinkResponse, DescopeError>) -> Void) {
        Task {
            do throws(DescopeError) {
    ...
    /// - Returns: An ``EnchantedLinkResponse`` object with the `linkId` to show the
    ///     user and `pendingRef` for polling for the session.
    func signUpOrInWithPhone(_ phone: String, redirectURL: String?, options: [SignInOptions], completion: @escaping @Sendable (Result<EnchantedLinkResponse, DescopeError>) -> Void) {
        Task {
            do throws(DescopeError) {
