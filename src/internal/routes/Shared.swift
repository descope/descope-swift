...
extension DescopeClient.JWTResponse {
    func convert() throws(DescopeError) -> AuthenticationResponse {
        ...
        return try AuthenticationResponse(sessionToken: Token(jwt: sessionJwt), refreshToken: Token(jwt: refreshJwt), user: user.convert(), isFirstAuthentication: firstSeen, externalToken: externalToken, flowOutput: flowOutput)
    }
}
