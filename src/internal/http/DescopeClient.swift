...
final class DescopeClient: HTTPClient, @unchecked Sendable {
    ...
    func enchantedLinkSignUp(loginId: String, details: SignUpDetails?, redirectURL: String?) async throws(DescopeError) -> EnchantedLinkResponse {
        return try await enchantedLinkSignUp(with: .email, loginId: loginId, details: details, redirectURL: redirectURL)
    }

    func enchantedLinkSignUpWithPhone(_ phone: String, details: SignUpDetails?, redirectURL: String?) async throws(DescopeError) -> EnchantedLinkResponse {
        return try await enchantedLinkSignUp(with: .sms, loginId: phone, details: details, redirectURL: redirectURL)
    }

    private func enchantedLinkSignUp(with method: DeliveryMethod, loginId: String, details: SignUpDetails?, redirectURL: String?) async throws(DescopeError) -> EnchantedLinkResponse {
    ...
    func enchantedLinkSignIn(loginId: String, redirectURL: String?, refreshJwt: String?, options: LoginOptions?) async throws(DescopeError) -> EnchantedLinkResponse {
        return try await enchantedLinkSignIn(with: .email, loginId: loginId, redirectURL: redirectURL, refreshJwt: refreshJwt, options: options)
    }

    func enchantedLinkSignInWithPhone(_ phone: String, redirectURL: String?, refreshJwt: String?, options: LoginOptions?) async throws(DescopeError) -> EnchantedLinkResponse {
        return try await enchantedLinkSignIn(with: .sms, loginId: phone, redirectURL: redirectURL, refreshJwt: refreshJwt, options: options)
    }

    private func enchantedLinkSignIn(with method: DeliveryMethod, loginId: String, redirectURL: String?, refreshJwt: String?, options: LoginOptions?) async throws(DescopeError) -> EnchantedLinkResponse {
        try await post("auth/enchantedlink/signin/\(method.rawValue)", headers: authorization(with: refreshJwt), body: [
            "loginId": loginId,
            "redirectUrl": redirectURL,
            "loginOptions": options?.dictValue,
        ])
    ...
    func enchantedLinkSignUpOrIn(loginId: String, redirectURL: String?, refreshJwt: String?, options: LoginOptions?) async throws(DescopeError) -> EnchantedLinkResponse {
        return try await enchantedLinkSignUpOrIn(with: .email, loginId: loginId, redirectURL: redirectURL, refreshJwt: refreshJwt, options: options)
    }

    func enchantedLinkSignUpOrInWithPhone(_ phone: String, redirectURL: String?, refreshJwt: String?, options: LoginOptions?) async throws(DescopeError) -> EnchantedLinkResponse {
        return try await enchantedLinkSignUpOrIn(with: .sms, loginId: phone, redirectURL: redirectURL, refreshJwt: refreshJwt, options: options)
    }

    private func enchantedLinkSignUpOrIn(with method: DeliveryMethod, loginId: String, redirectURL: String?, refreshJwt: String?, options: LoginOptions?) async throws(DescopeError) -> EnchantedLinkResponse {
        try await post("auth/enchantedlink/signup-in/\(method.rawValue)", headers: authorization(with: refreshJwt), body: [
            "loginId": loginId,
            "redirectUrl": redirectURL,
            "loginOptions": options?.dictValue,
        ])
